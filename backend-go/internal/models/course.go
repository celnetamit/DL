...
type Course struct {
	...
	Award       *CourseAward `gorm:"-" json:"award,omitempty"`
}
...
type CourseAward struct {
	...
	UserID          string    `gorm:"type:uuid;not null" json:"user_id"`
	CourseID        string    `gorm:"type:uuid;not null" json:"course_id"`
	BadgeSlug       string    `gorm:"not null;default:course-complete" json:"badge_slug"`
	BadgeLabel      string    `gorm:"not null;default:Course Complete" json:"badge_label"`
	CertificateCode string    `gorm:"not null;uniqueIndex" json:"certificate_code"`
	IssuedAt        time.Time `gorm:"not null;default:now()" json:"issued_at"`
}

func (CourseAward) TableName() string {
	return "course_awards"
}
