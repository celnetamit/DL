...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    apiFetch<ProductContentsResponse>(`/api/v1/products/${id}/contents`, { cache: "no-store" })
      .then((res) => {
        setData(res);
  ...
  const totalCount = modules.reduce((sum, m) => sum + m.contents.length, 0);
        ...
        <div className="glass rounded-3xl p-8">
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {product.content_types?.map((ct) => (
              <span key={ct} className="text-[10px] uppercase tracking-widest bg-dune/10 text-dune/60 rounded-full px-2 py-0.5">
                {TYPE_ICONS[ct] || "📁"} {ct}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div>
              <p className="text-xs text-dune/40 uppercase tracking-widest">Total Contents</p>
              <p className="text-2xl font-bold font-[var(--font-space)]">{totalCount}</p>
            </div>
            <div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-dune/40">{mod.contents.length} item{mod.contents.length !== 1 ? "s" : ""}</p>
                      </div>
                    </div>
